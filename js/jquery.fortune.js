  ...
  $.fn.fortune = function (args) {
    ...
    fortune.spin = function (price) {
      ...
      if (email == undefined || email == "") {
        price = arrWin[Math.floor(Math.random() * arrWin.length)];
      }
      else if (numberWin < 3) {
        // price = typeof price === "number"?price:Math.floor(Math.random() * prices_amount);
        price = arrPriceAll[Math.floor(Math.random() * arrPriceAll.length)];
      } else {
        price = arrFail[Math.floor(Math.random() * arrFail.length)];
      }
